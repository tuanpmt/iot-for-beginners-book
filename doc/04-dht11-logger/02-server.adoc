...
<5> Khởi tạo một server HTTP và mở port 8000 để client truy cập
...
<2> Tạo 2 textbox tại tag tiêu đề phụ <h1> để hiển thị nhiệt độ, độ ẩm. Mỗi textbox sẽ có 1 id để ta có thể cập nhật giá nhiệt độ, độ ẩm từ server. Mã định dạng `&#176;C` là của kí tự độ C

<3> Phần còn lại sử dụng các thuộc tính của thư viện HTML5 Canvas JavaScript để có thể vẽ đồ thị nhiệt độ, độ ẩm
