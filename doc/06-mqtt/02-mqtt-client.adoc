...
Các options:
	...
	* `-v` : verbose, option cho phép ghi lại nhất kí hoạt động của các tập tin trong file cấu hình.
...
Tiếp theo, chúng ta sẽ tìm hiểu cách sử dụng thư viện ESP8266MQTTClient, thư viện được cộng đồng developer đánh giá là ổn định dễ sử dụng hơn so với thư viện PubSubClient.
...
Tương tự như mã nguồn của chương trình sử dụng thư viện pubsubCLient, chúng ta cũng sẽ khởi tạo ESP8266 là mqtt client trong class MQTT của thư viện ESP8266MQTTClient. Cài đặt ESP8266 ở chế độ station và kết nối đến network wifi. Chức năng của các hàm trong thư viện đã được giải thích ở file mã nguồn, ở hàm `mqtt.onConnect()` chúng ta sẽ subscribe 2 topic là `hello/ESP8266` và `MQTTlens/test/#`, dấu # sẽ giúp nhận dữ liệu của tất cả các topic có nội dung phần đầu là `MQTTlens/test/`. Hàm `mqtt.onSubscribe()` sẽ puslish các message ở topic đã chỉ định. `mqtt.onData()` sẽ nhận, kiểm tra và xử lí dữ liệu nhận được từ topic đã subscribe. Ở `loop()` chúng ta chỉ cần gọi hàm `handle()` để khởi tạo và kiểm tra các thuộc tính của giao thức cũng như xử lí, truyền và nhận dữ liệu từ các topic đã subscribe và public.
...
* **Bước 3 :** Mở MQTT lens trên trình duyệt Chrome, tạo 1 connection với host name `iot.eclipse.org`, sử dụng port 1883. Thực hiện subscribe topic `MQTTlens/test/3`, pushlish các message vào topic `hello/ESP8266`. Nếu pushlish message với nội dung `on`, led GPIO16 trên board sẽ sáng, pushlish message `off` led GPIO16 trên board sẽ tắt. Các message với nội dung khác thì vẫn sẽ hiển thị dữ liệu nhận được trên serial terminal của Arduino nhưng sẽ không có tác dụng điều khiển led GPIO16. Kết quả hiển thị như hình bên dưới:

.Kết quả hiển thị trên serial terminal và MQTT lens khi sử dụng thư viện ESP8266MQTTClient
image::06-mqtt/ESP8266MQTTClient.png[width=897, align="center"]

Ngoài ra có thể tạo thêm 1 connection và pulish các message vào topic `MQTTlens/test/#` với # có thể là 1 nội dung bất kì. Ở Serial terminal của Arduino, chúng ta sẽ nhìn thấy nội dung của message gửi bởi MQTT lens bởi vì ESP8266 đã đăng kí nhận gói tin tại topic `MQTTlens/test/#`
