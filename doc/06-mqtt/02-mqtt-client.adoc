...
Chúng ta sẽ tạo một biến `espClient` thuộc lớp `WiFiClient`, biến này được khai báo là client và sử dụng các thuộc tính của thư viện `PubSubClient`. Tại hàm setup() sẽ thiết lập ESP8266 ở chế độ station, kết nối đến mạng wifi. Bên cạnh đó, function setup() sẽ tự động kết nối lại với broker MQTT khi xảy ra mất kết nối đồng thời thực hiện các chức năng publish, subscribe của 1 MQTT client thông qua hàm `reconnect()` . Function `callback()` sẽ lấy dữ liệu của các puslisher khi publish 1 message và gửi đến các client đã subscribe topic đó, đông thời sẽ kiểm tra nội dung của message để điều khiển led ở GPIO16. Hàm `loop()` có chức năng kết nối client là ESP8266 với broker, thực  hiện chức năng publish 1 message và subscribe topic. Đối tượng `client.loop()` kiểm tra thời gian kết nối của client với gói `KEEP_ALIVE` để đưa ra các thông tin về trạng thái kết nối của ESP8266 đồng thời lấy dữ liệu của message từ buffer để gửi đến các client đã subcribe topic. 


* **Bước 3 :** Mở MQTT lens trên trình duyệt Chrome, tạo 1 connection với host name `broker.mqtt-dashboard.com`, sử dụng port 1883. Thực hiện subscribe topic `ESP8266/connection/board`. Sau khi nhấn nút subscribe trên MQTT lens sẽ xuất hiện 1 message gửi từ esp8288 với nội dung `connnected`. Thực hiện pushlish các message vào topic `ESP8266/LED_GPIO16/status`. Nếu pushlish message với nội dung `on`, led GPIO16 trên board sẽ sáng, pushlish message `off` led GPIO16 trên board sẽ tắt. Các message với nội dung khác sẽ không có tác dụng điều khiển led GPIO16. Kết quả hiển thị như hình bên dưới:
...
* **Bước 3 :** Download ứng dụng `IoT Smartconfig`. Search ứng dụng với từ khóa `iot smartconfig` (hỗ trợ hệ điều hành Android và iOS), cài đặt ứng dụng như hình dưới.
...
image::06-mqtt/interfaceConfig.png[width=300, align="center"]

Giải thích: 
Tại mục host name dùng `ws://iot.eclipse.org:80/ws` với

* `ws`(websocket) là phương thức gửi, nhận dữ liệu trong đó chúng  sẽ được tìm hiểu chi tiết ở bài học kế tiếp  
* `iot.eclipse.org` broker MQTT để gửi, nhận các gói tin từ client
* `80` port được mở để sử dụng websocket
