...
uint32_t ESP.getFreeHeap(); //trả về kích thước vùng nhớ trống ở heap
...
WiFi.beginSmartConfig();//bắt đầu thiết lập chế độ smart config

/* Soft AP */
WiFi.softAP(ssid, password);//khởi tạo 1 network AP
//thiết lập cấu hình cho AP gồm địa chỉ IP, gateway và subnet
WiFi.softAPConfig (local_ip, gateway, subnet);
WiFi.softAPgetStationNum(); //trả về số station đã kết nối đến
WiFi.softAPdisconnect(wifioff);//ngắt kết nối của các station
...
*/
WiFi.printDiag(Serial); //in ra serial các chuẩn đoán thông tin của network
Serial.setDebugOutput(true); //enable chế độ Wi-Fi Diagnostic */
